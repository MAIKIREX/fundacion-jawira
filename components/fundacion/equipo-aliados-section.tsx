...
export default function EquipoAliadosSection() {
  ...
  useGSAP(() => {
    ...
    mm.add("(prefers-reduced-motion: no-preference)", () => {
      ...
      headerTl
        .from('.equipo-subtitle', { y: 20, autoAlpha: 0, duration: 0.6, ease: 'power3.out' })
        .from('.equipo-title', { y: 30, autoAlpha: 0, duration: 0.8, ease: 'power3.out' }, '-=0.4')
        .from('.equipo-desc', { y: 20, autoAlpha: 0, duration: 0.6, ease: 'power2.out' }, '-=0.4')
  ...
  return (
    <section ref={sectionRef} className="py-24 md:py-32 bg-background border-t border-border/20 overflow-hidden">
          ...
          <div className="equipo-header mb-16 flex flex-col lg:flex-row lg:items-end justify-between gap-8 lg:gap-16">
            <div className="max-w-2xl">
              ...
              <h2 className="equipo-title text-4xl md:text-5xl lg:text-6xl font-black text-foreground tracking-tighter leading-[1.1]">
                Profesionales que<br className="hidden md:block" />
                impulsan el cambio.
              </h2>
            </div>
            <p className="equipo-desc text-lg text-muted-foreground leading-relaxed max-w-[40ch] lg:pb-2">
              La Fundación JAWIRA cuenta con un equipo interdisciplinario compuesto
              por expertos en salud, educación, tecnología e investigación.
            </p>
          </div>
            ...
            >
              <CarouselContent className="-ml-6 md:-ml-8">
                {teamMembers.map((member, i) => (
                  <CarouselItem key={i} className="pl-6 md:pl-8 basis-[80%] sm:basis-[320px] md:basis-[380px]">
        ...
        <div className="aliados-section pt-10 md:pt-20">
          ...
          <div className="mb-12 max-w-2xl">
            ...
            <p className="aliados-header-item text-lg text-muted-foreground mt-4 leading-relaxed max-w-[50ch]">
              La Fundación JAWIRA trabaja en alianza estratégica con instituciones
              públicas, privadas y comunitarias para amplificar nuestro impacto.
            </p>
          </div>

          {/* Allies grid */}
          <div className="ally-grid grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {allies.map((ally, idx) => (
              <div
                key={idx}
                className="ally-card h-28 flex items-center justify-center bg-muted/20 rounded-[1.5rem] border border-border/30 hover:border-border/60 hover:bg-muted/40 transition-colors px-6 group"
              >
                {ally.image ? (
                  <div className="relative w-full h-12 grayscale opacity-50 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-500">
                    <Image
                      src={ally.image}
                      alt={ally.nombre}
                      fill
          ...
          {/* Alliance CTA */}
          <div className="mt-8">
            <Link href="/contacto" className="inline-flex items-center gap-2 text-sm font-bold text-muted-foreground hover:text-foreground transition-colors group">
